...
# Configure logging level for research module to see candidate-ranking logs
logging.getLogger("research").setLevel(logging.INFO)

# Also set root logger to INFO to see all logs
logging.basicConfig(
    level=logging.INFO,
...
async def write_report(research_request: ResearchRequest, research_id: str = None):
    ...
    if research_request.report_type != "multi_agents":
        ...
        if research_request.report_type in [ReportType.DeepResearch.value, "deep_research"]:
            if "## Deep Research Settings" not in report:
                breadth = None
                depth = None
                if researcher and getattr(researcher, "deep_researcher", None):
                    breadth = getattr(researcher.deep_researcher, "breadth", None)
                    depth = getattr(researcher.deep_researcher, "depth", None)
                if breadth is None and researcher:
                    breadth = getattr(researcher.cfg, "deep_research_breadth", None)
                if depth is None and researcher:
                    depth = getattr(researcher.cfg, "deep_research_depth", None)
    ...
    if research_request.report_type != "multi_agents":
        ...
        json_path = ""
        try:
            if researcher and getattr(researcher, "websocket", None):
                handler = researcher.websocket
                if hasattr(handler, "generate_task_graph"):
                    # Prepare research configuration to pass to graph generator
                    research_config = {}
                    if researcher and getattr(researcher, "deep_researcher", None):
                        research_config = {
                            "depth": getattr(researcher.deep_researcher, "depth", None),
                            "breadth": getattr(researcher.deep_researcher, "breadth", None),
                            "concurrency": getattr(researcher.deep_researcher, "concurrency_limit", None),
                            "report_type": research_request.report_type
                        }
                    elif research_request.report_type in [ReportType.DeepResearch.value, "deep_research"]:
                        # Fallback to config values if deep_researcher not available
                        research_config = {
                            "depth": getattr(researcher.cfg, "deep_research_depth", None),
                            "breadth": getattr(researcher.cfg, "deep_research_breadth", None),
                            "concurrency": getattr(researcher.cfg, "deep_research_concurrency", None),
                            "report_type": research_request.report_type
                        }

                    graph_fs_path = handler.generate_task_graph(research_config)
                    graph_path = urllib.parse.quote(os.path.relpath(graph_fs_path))
                if hasattr(handler, "log_file"):
                    json_path = urllib.parse.quote(os.path.relpath(handler.log_file))
        except Exception:
            graph_path = ""
            json_path = ""
        response = {
