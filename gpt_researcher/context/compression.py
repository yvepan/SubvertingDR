...
class VectorstoreCompressor:
    ...
    async def async_get_context_with_sources(self, query: str, max_results: int = 5) -> tuple[str, list[str]]:
        results = await self.vector_store.asimilarity_search(query=query, k=max_results, filter=self.filter)
        sources = [
            d.metadata.get("source")
            for d in results
            if isinstance(d.metadata, dict) and d.metadata.get("source")
        ]
        return self.prompt_family.pretty_print_docs(results), sources
...
class ContextCompressor:
    ...
    async def async_get_context_with_sources(
        ...
        if cost_callback:
            cost_callback(estimate_embedding_cost(model=OPENAI_EMBEDDING_MODEL, docs=self.documents))
        relevant_docs = await asyncio.to_thread(compressed_docs.invoke, query, **self.kwargs)
        selected_docs = relevant_docs if max_results is None else list(relevant_docs)[:max_results]
        sources = [
            d.metadata.get("source")
            for d in selected_docs
            if isinstance(d.metadata, dict) and d.metadata.get("source")
        ]
        return self.prompt_family.pretty_print_docs(selected_docs), sources
