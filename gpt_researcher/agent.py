...
from .actions.utils import calculate_cost, stream_output
...
class GPTResearcher:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._current_phase = "research"  # Track current research phase: "planning", "research", or "summary"
        ...
        if self.log_handler:
            try:
                if event_type == "tool":
                    ...
                    kwargs_method = {k: v for k, v in kwargs.items() if k != 'tool_name'}
                    await self.log_handler.on_tool_start(tool_name, **kwargs_method)
                elif event_type == "action":
                    action_val = kwargs.get('action', '')
                    kwargs_method = {k: v for k, v in kwargs.items() if k != 'action'}
                    await self.log_handler.on_agent_action(action_val, **kwargs_method)
    ...
    async def write_report(self, existing_headers: list = [], relevant_written_contents: list = [], ext_context=None, custom_prompt="") -> str:
        ...
        # Set phase to summary for report generation
        original_phase = self._current_phase
        self._current_phase = "summary"
        ...
        if self.report_type in [ReportType.DeepResearch.value, "deep_research"]:
            breadth = getattr(self.deep_researcher, "breadth", getattr(self.cfg, "deep_research_breadth", None))
            depth = getattr(self.deep_researcher, "depth", getattr(self.cfg, "deep_research_depth", None))
            if breadth is not None and depth is not None:
                report = f"## Deep Research Settings\n\n- Depth: {depth}\n- Width: {breadth}\n\n{report}"
    ...
    def add_costs(self, cost: float | dict, phase: str = None) -> None:
        """Add to the accumulated API costs and log usage details if available.

        Args:
            cost: Either a numeric cost (USD) or a dict with keys
                  cost, prompt_tokens, completion_tokens, elapsed_ms, model.
            phase: The research phase - one of: "planning", "research", "summary"
                   If not provided, uses the current phase from self._current_phase
        """
        # Use current phase if not explicitly provided
        if phase is None:
            phase = self._current_phase
        if isinstance(cost, (float, int)):
            amount = float(cost)
            self.research_costs += amount
            if self.websocket:
                import asyncio
                details = {
                    "cost": amount,
                    "total_cost": self.research_costs,
                    "task": self.query,  # Add task identifier
                    "parent_query": self.parent_query,  # Add parent query
                    "phase": phase  # Add phase identifier
                }
                try:
                    asyncio.create_task(stream_output(
                        "logs",
                        "cost_update",
                        f"Cost updated: ${amount}",
                        self.websocket,
                        True,
                        details
                    ))
        ...
        if isinstance(cost, dict):
            if "cost" in cost and isinstance(cost.get("cost"), (float, int)):
                amount = float(cost.get("cost", 0.0))
            else:
                pt = cost.get("prompt_tokens")
                ct = cost.get("completion_tokens")
                model = cost.get("model", "")
                amount = calculate_cost(pt or 0, ct or 0, model) if (pt is not None and ct is not None) else 0.0
            self.research_costs += amount
            details = {
                "cost": amount,
                "total_cost": self.research_costs,
            ...
            if "prompt_tokens" in cost:
                details["prompt_tokens"] = cost["prompt_tokens"]
            if "completion_tokens" in cost:
                details["completion_tokens"] = cost["completion_tokens"]
            if "elapsed_ms" in cost:
                details["elapsed_ms"] = cost["elapsed_ms"]
            if "model" in cost:
        ...
        raise ValueError("Cost must be a number or dict")
