...
class ResearchConductor:
    ...
    def _format_local_url(self, url: str) -> str:
        """
        Format URL for display. Local documents are already using virtual URLs.
        This function now simply returns the URL as-is.
        """
        return url
    ...
    async def conduct_research(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                self.researcher.websocket,
                metadata={"task": self.researcher.query, "parent_query": self.researcher.parent_query}
            )
            await stream_output(
                "logs",
                "agent_generated",
        ...
        if self.researcher.source_urls:
            ...
            if document_data and self.researcher.verbose:
                for doc in document_data:
                    if isinstance(doc, dict) and 'url' in doc:
                        formatted = self._format_local_url(doc['url'])
                        await stream_output(
                            "logs",
                            "added_source_url",
                            f"📄 Loaded local document: {formatted}\n",
                            self.researcher.websocket,
                            True,
                            formatted,
                        )
        ...
        # Hybrid search including both local documents and web sources
            ...
            # Local poison: load documents directly
            local_poison_data = []
            if self.researcher.document_urls:
                local_poison_data = await OnlineDocumentLoader(self.researcher.document_urls).load()
            elif hasattr(self.researcher.cfg, 'doc_path_local_poison') and self.researcher.cfg.doc_path_local_poison:
                local_poison_data = await DocumentLoader(self.researcher.cfg.doc_path_local_poison).load()
                self.logger.info(f"Local poison: loaded {len(local_poison_data)} documents from {self.researcher.cfg.doc_path_local_poison}")
            elif self.researcher.cfg.doc_path:
                local_poison_data = await DocumentLoader(self.researcher.cfg.doc_path).load()

            if local_poison_data and self.researcher.verbose:
                for doc in local_poison_data:
                    if isinstance(doc, dict) and 'url' in doc:
                        formatted = self._format_local_url(doc['url'])
                        await stream_output(
                            "logs",
                            "added_source_url",
                            f"📄 Local poison - directly loaded document: {formatted}\n",
                            self.researcher.websocket,
                            True,
                            formatted,
                        )
            ...
            if self.researcher.vector_store:
                self.researcher.vector_store.load(local_poison_data)

            # Web poison: mix virtual URLs into search
            # This is handled automatically in _search_relevant_source_urls

            # Call _get_context_by_web_search once while handling local and web inputs
            # Local-poison documents are passed as scraped_data; web poison is mixed in during search
            research_data = await self._get_context_by_web_search(self.researcher.query, local_poison_data, self.researcher.query_domains)
    ...
    async def _process_sub_query(self, sub_query: str, scraped_data: list = [], query_domains: list = []):
        ...
        try:
            ...
            # Separate local/pre-loaded documents from web search results
            local_scraped_data = scraped_data if scraped_data else []
            web_scraped_data = []

            # Always perform web search for sub_query (unless in pure Local mode)
            if self.researcher.report_source != ReportSource.Local.value:
                web_scraped_data = await self._scrape_data_by_urls(sub_query, query_domains)
                self.logger.info(f"Web scraped data size: {len(web_scraped_data)}")

            # Combine local and web scraped data
            all_scraped_data = local_scraped_data + web_scraped_data
            self.logger.info(f"Total scraped data size: {len(all_scraped_data)} (local: {len(local_scraped_data)}, web: {len(web_scraped_data)})")

            # Get similar content based on all scraped data
            if all_scraped_data:
                # Record source URLs from scraped_data (including local files)
                for item in all_scraped_data:
                    if isinstance(item, dict) and 'url' in item:
                        url = item['url']
                        formatted = self._format_local_url(url)
                        is_local = formatted.startswith("https://scholar.local/documents/")
                        is_web_poison = url.startswith("http://research/")
                        key = formatted if is_local else url

                        # Record in visited_urls if needed
                        if key and key not in self.researcher.visited_urls:
                            self.researcher.visited_urls.add(key)

                        # Always log for the task execution tree
                        if self.researcher.verbose and key:
                            label = "local poison document" if is_local else ("web poison document" if is_web_poison else "web source")
                            await stream_output(
                                "logs",
                                "selected_source_url",
                                f"✅ Selected source ({label}): {key}\n",
                                self.researcher.websocket,
                                True,
                                key,
                            )

                web_context = await self.researcher.context_manager.get_similar_content_by_query(sub_query, all_scraped_data)
    ...
    async def _get_local_document_urls_with_titles(self):
        """
        Web poison: get virtual URLs and titles for mixing into web search results.
        Loads documents from DOC_PATH_WEB_POISON.
        Returns a list of dicts with format: [{"url": "http://research/{filename}", "title": "filename"}]
        """
        local_url_items = await get_local_document_url_items(self.researcher)
        return [
            {
            ...
            for item in local_url_items
        ]

    async def _get_local_document_urls(self):
        """
        Web poison: get virtual URLs for mixing into web search results.
        Loads documents from DOC_PATH_WEB_POISON. Kept for compatibility with the old interface.
        Returns a list of virtual URLs in format: http://research/{filename}
        """
        url_items = await self._get_local_document_urls_with_titles()
        return [item["url"] for item in url_items]

    async def _filter_urls_by_title_similarity(self, query: str, url_items: list, dynamic_top_k: bool = True, summary_max_chars: int | None = None) -> list:
        """
        Filter URLs based on title/query similarity with optional dynamic top-k.

        Args:
            query: Search query.
            url_items: URL items in [{"url": "...", "title": "...", "body": "..."}] format.
            dynamic_top_k: Whether to use the dynamic top-k strategy.
                - True: set top-k from the number of real URLs.
                  * virtual URLs do not increase the top-k limit.
                - False: skip filtering and return all URLs.
            summary_max_chars: Maximum body characters used as a summary; None means no truncation.
        ...
        return [item.get("url") or item.get("href") for item in filtered_items]

    async def _search_relevant_source_urls(self, query, query_domains: list | None = None, parent_query: str = ""):
        """
        Search relevant source URLs with optional title-similarity filtering.
        """
        all_url_items = []  # Store URL items with titles
        if query_domains is None:
            query_domains = []
        ...
        for retriever_class in self.researcher.retrievers:
            ...
            try:
                ...
                # Collect search results while preserving full metadata
        ...
        # Add local document URLs if in hybrid or local mode
        self.logger.info(f"Checking report_source: researcher.report_source={self.researcher.report_source}, cfg.report_source={self.researcher.cfg.report_source}")
        if self.researcher.report_source in ["local", "hybrid"]:
            local_url_items = await self._get_local_document_urls_with_titles()
            all_url_items.extend(local_url_items)
            self.logger.info(f"Web poison: added {len(local_url_items)} virtual URLs to search results")

        # Check whether candidate ranking is enabled; this only affects source ranking
        enable_similarity_filter = getattr(self.researcher.cfg, 'enable_url_similarity_filter', False)

        if enable_similarity_filter:
            # Use dynamic top-k for candidate ranking
            similarity_query = build_root_query_anchored_query(parent_query, query)
            self.logger.info(f"[Research stage] node subtask={parent_query[:60] if parent_query else '(none)'} internal retrieval task={query[:60]} combined query={similarity_query[:80]}")
            self.logger.info(f"Candidate ranking enabled (total URLs: {len(all_url_items)})")
            filtered_urls = await self._filter_urls_by_title_similarity(similarity_query, all_url_items, dynamic_top_k=True, summary_max_chars=200)
        else:
            # Skip ranking and return all URLs
            self.logger.info("Candidate ranking disabled; using all URLs")
            filtered_urls = [item["url"] for item in all_url_items]

        # Get unique URLs
        new_search_urls = await self._get_new_urls(filtered_urls)
    ...
    async def _scrape_data_by_urls(self, sub_query, query_domains: list | None = None):
        ...
        new_search_urls = await self._search_relevant_source_urls(
            sub_query, query_domains, parent_query=self.researcher.query
        )
