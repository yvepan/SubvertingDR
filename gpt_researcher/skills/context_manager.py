...
class ContextManager:
    ...
    async def get_similar_content_by_query(self, query: str, pages: list) -> str:
        ...
        context, sources = await context_compressor.async_get_context_with_sources(
            query=query, max_results=10, cost_callback=self.researcher.add_costs
        )
        if self.researcher.verbose and sources:
            import os, re
            for url in dict.fromkeys(sources):
                ...
                if isinstance(url, str) and url.lower().startswith("file://"):
                    ...
                    formatted = f"https://docs.jarvislabs.ai/blog/{slug}"
                await stream_output(
                    "logs",
                    "selected_source_url",
                    f"✅ Selected source for context: {formatted}\n",
                    self.researcher.websocket,
                    True,
                    {"url": formatted, "task": query},
                )
        return context
    ...
    async def get_similar_content_by_query_with_vectorstore(self, query: str, filter: dict | None) -> str:
        ...
        context, sources = await vectorstore_compressor.async_get_context_with_sources(query=query, max_results=8)
        if self.researcher.verbose and sources:
            import os, re
            for url in dict.fromkeys(sources):
                formatted = url
