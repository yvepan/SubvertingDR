...
class GenericLLMProvider:
    ...
    async def get_chat_response_with_usage(self, messages, stream, websocket=None, **kwargs):
        if not stream:
            output = await self.llm.ainvoke(messages, **kwargs)
            ...
            if isinstance(meta, dict):
                if "token_usage" in meta and isinstance(meta["token_usage"], dict):
                    tu = meta["token_usage"]
                    # Try both input_tokens/output_tokens (Anthropic style) and prompt_tokens/completion_tokens (OpenAI style)
                    usage["prompt_tokens"] = tu.get("input_tokens") or tu.get("prompt_tokens")
                    usage["completion_tokens"] = tu.get("output_tokens") or tu.get("completion_tokens")
                else:
                    # Try both naming conventions
                    usage["prompt_tokens"] = meta.get("input_tokens") or meta.get("prompt_tokens")
                    usage["completion_tokens"] = meta.get("output_tokens") or meta.get("completion_tokens")
            return {"content": content, "usage": usage}
        else:
            streamed = await self.stream_response_with_usage(messages, websocket, **kwargs)
            return streamed
    ...
    async def stream_response_with_usage(self, messages, websocket=None, **kwargs):
        paragraph = ""
        response = ""
        last_meta = None
        ...
        async for chunk in self.llm.astream(messages, **kwargs):
            ...
            if hasattr(chunk, "response_metadata"):
                last_meta = getattr(chunk, "response_metadata", None)
            if content is not None:
                response += content
                paragraph += content
                if "\n" in paragraph:
        ...
        if isinstance(last_meta, dict):
            if "token_usage" in last_meta and isinstance(last_meta["token_usage"], dict):
                tu = last_meta["token_usage"]
                # Try both input_tokens/output_tokens (Anthropic style) and prompt_tokens/completion_tokens (OpenAI style)
                usage["prompt_tokens"] = tu.get("input_tokens") or tu.get("prompt_tokens")
                usage["completion_tokens"] = tu.get("output_tokens") or tu.get("completion_tokens")
            else:
                # Try both naming conventions
                usage["prompt_tokens"] = last_meta.get("input_tokens") or last_meta.get("prompt_tokens")
                usage["completion_tokens"] = last_meta.get("output_tokens") or last_meta.get("completion_tokens")
